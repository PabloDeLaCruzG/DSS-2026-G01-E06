...
@section('content')
    ...
    <section class="mb-8">
        <form action="{{ route('home') }}" method="GET">
            ...
            <div class="flex items-center gap-3 bg-gray-800 border border-gray-700 rounded-lg px-4 py-2.5 mb-4">
                <button type="submit" class="text-gray-500 hover:text-violet-400 transition-colors text-sm">🔍</button>
                <input
                    type="text"
                    name="search"
    ...
    <section class="mb-10">
        <div class="flex items-center justify-between mb-4">
            ...
            <a href="{{ route('home', ['rating' > '4.5']) }}" class="text-xs text-violet-400 hover:text-violet-300 transition-colors">View all →</a>
        </div>

            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                @forelse($games as $game)
<a href="{{ route('games.show', $game->id) }}"
   class="bg-gray-800 border border-gray-700 rounded-xl overflow-hidden hover:border-violet-500 transition-colors group block">
